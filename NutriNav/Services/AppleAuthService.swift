...
class AppleAuthService: NSObject, AuthService, ObservableObject {
    ...
    func continueAsGuest() {
        let guestUser = AuthUser(id: "GUEST_USER_\(UUID().uuidString)", email: "guest@nutrinav.app", fullName: "Guest")
        Task { @MainActor in
    ...
    func handleAuthorizationSuccess(authorization: ASAuthorization, continuation: CheckedContinuation<AuthUser, Error>) {
    ...
    func handleAuthorizationFailure(error: Error, continuation: CheckedContinuation<AuthUser, Error>) {
